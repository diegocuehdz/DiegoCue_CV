...
\newcommand \EngineeringTitle {Tech Lead, iOS \& React Native Engineering}
...
\newcommand \Freelance {Consulting \& Contractor}
...
\newcommand \PExpSeniorNodeDevDescription {Senior Backend Developer (NodeJS \& GraphQL)}
...
\newcommand \CertificationsAirshipMobileRange {December 2025}
\newcommand \CertificationsAirshipMobileDescription {Mobile Certification}
\newcommand \CertificationsAirshipMobileProvider {Airship}
\newcommand \CertificationsAirshipMobileCertID {ID: o553vvdqkgss}
\newcommand \CertificationsAirshipMobileURL {\url{https://verify.skilljar.com/c/o553vvdqkgss}}

\newcommand \CertificationsTTTProEssentialsRange {January 2025}
...
\newcommand \SkillsLanguagesDescription {Typescript, Swift, Objective-C, C\#}
...
\newcommand \SkillsDesignDescription {Figma, Sketch, Photoshop}
...
\newcommand \SkillsCloudComputingDescription {Cloudflare Dev. Platform, AWS, GCP}
\newcommand \SkillsAutomatizationSubTitle {Automatization}
\newcommand \SkillsAutomatizationDescription {Github Actions, Fastlane, Jenkins}
...
\newcommand \ProjectsNoticiasRange {2025-today}
\newcommand \ProjectsNoticiasTitle {Univision Noticias}
\newcommand \ProjectsNoticiasSubTitle {News \& Entertainment mobile app}
\newcommand \ProjectsNoticiasRole {Tech Lead, Lead iOS Developer \& Lead React Native Developer}
\newcommand \ProjectsNoticiasURL {\url{https://www.univision.com/noticias}}
\newcommand \ProjectsNoticiasDescription {For this project I was in charge of handling the migration \& revamp for Univision's Noticias app in the company's shared private/white label monorepo. This project continues on the foundations that TUDN set, UI library and shared libraries were reused and adapted as much as possible while shipping a new and improved version with optimized functionality prioritizing user content consumption and business KPIs. Part of my day to day was used in grooming \& planning the architectural features that needed to be migrated as well as design the new features that needed to be developed from scratch. The project had the following characteristics:
\begin{itemize}
    \item Usage of latest toolchain suite using biome, oxlint, oxfmt, turborepo, GraphQL, Expo with Expo Modules and Native Views
    \item Integrations with company sensitive APIs and SDK like IBM's Weather Company, Fastly VCL custom source code \& aggressive caching at a CDN layer using Akamai
    \item Compliant implementation of PII data using OneTrust CMP, guaranteeing multiple compliance for different state laws across the digital product
\end{itemize}
}
...
\newcommand \ProjectsTUDNRole {Tech Lead, Lead iOS Developer \& Lead React Native Developer}
\newcommand \ProjectsTUDNURL {\url{https://tudn.com/}}
\newcommand \ProjectsTUDNDescription {For this project I was in charge of handling the migration for TUDN's new app in the company's new stack. This project was handled via monorepo setup with shared components, utilities and architecture with the web teams. I lead a team of 5+ engineers in doing the app from scratch by using the latest technologies. Part of my day to day was used in planning features with design, product and QA teams in order to reach the goals and dates for the migration. The project had some of the following characteristics:
\begin{itemize}
    \item Latest tooling suite using oxlint, turborepo, GraphQL, ReactNative with Expo dependencies and Native Modules
    \item Integrations with analytics tools like kochava, appsflyer, google analytics and statsig
    \item Integrations with diverse ads functionality. Handled different ads specs for display and video ads
    \item Custom player implementations using JWPlayer and Akta player by using complex content syndication through CDNs
\end{itemize}
}
...
\newcommand \ProjectsADomicilioRole {Mobile Architect, Product \& Project Manager}
\newcommand \ProjectsADomicilioURL {\url{https://adomicil.io/}}
\newcommand \ProjectsADomicilioDescription {In this project I was in charge of developing, architecturing and engineering the mobile Apps of aDomicil.io, which is a product that focuses on handling the delivery channel of restaurants primarily in Mexico City. I was also in charge of coordinating the project development with cross functional teams. Part of my role as engineer and product/project manager included in designing the optimal interfaces, and data transfer for the project needs and passing the requirements to technological teams. This project had modules and integrations with third party APIs like:
\begin{itemize}
...
\newcommand \ProjectsDaimlerDescription {In this project I developed the iOS Application for Daimler/Mercedes Benz which helped employees through out the company manage their car ownership and leasing. This tedious process was done manually and without technology before we developed the technological solution. By doing this digitalization and optimization of their processes' we helped them save valuable time through out the whole chain of operation of this tasks. This project gave visibility and traceability through out the whole organization which helped in internal and external auditing. The project had modules like:
\begin{itemize}
